...
def readlogs(path):
                ...
                try:
                    ...
                    with open(filepath, 'r',encoding='utf-8') as f:  #打开文件
                        ...
                        if('ERROR' in last_line or 'error' in last_line):
                            ...
                            diff_times=-1
                            error_flag='ERROR'
                        elif (' not exist' in last_line or 'exist, wait' in last_line):
                            #diff_times=-1
                            error_flag='Waiting'
                            filepath=last_line
                        else:
    ...
    maxs=tables_list.groupby('tables_name')['tables_name','start_time'].max()
    tables_list=tables_list.merge(maxs,how='inner',on=['tables_name','start_time'])
    return tables_list
...
def readShell(path):
    ...
    except_list=['cdi_area_daily.sh','sqoop_test.sh','auto_hive_deploy.sh']
    nums=1
    for i in os.walk(path):
        for files in i[2]:
            filepath=i[0]+'/'+files
...
def getHive():
    ...
    sql_txt="""
            ...
            LEFT JOIN(select tbl_id,
                            ...
                            max(if(PARAM_KEY='numRows',PARAM_VALUE,null)) row_num,
                            max(if(PARAM_KEY='rawDataSize',PARAM_VALUE,null)) raw_data_size,
                            max(if(PARAM_KEY='totalSize',PARAM_VALUE,null)) total_size,
