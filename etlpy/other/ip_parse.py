...
def get_ipaddr(ip):  
    ...
    if jsondata['code'] == 1:
        ...
        return  ipdata 
def get_ipaddr_html(ip):  
     
    API = "http://www.ip138.com/ips1388.asp?ip={0}&action=2"
    url = API.format(ip)
    req =Request(url)  
    req.add_header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36")  
    #req.add_header("GET",url)  
    req.add_header("Host","www.ip138.com")  
    req.add_header("Referer",url)  
    conent=urlopen(req).read().lower()#.decode('utf-8')
    load_time=time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())) 
    try:
        page = etree.HTML(conent)
        print(page)
        dt = page.xpath("//ul[@class='ul1']") #帖主时发帖间
        for i in dt:
            for j in i:
                print(j.text)
    except :
