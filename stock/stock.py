...
if __name__ == '__main__':
    ...
    parser.add_option('-c', '--stock-code', dest='codes', help="the stock's code that you want to query.")
    parser.add_option('-s', '--sleep-time', dest='sleep_time', default=6, type="int", help='How long does it take to check one more time.')
    parser.add_option('-t', '--thread-num', dest='thread_num', default=3, type='int', help="thread num.")
    options, args = parser.parse_args(args=['sh600339','sh600556'])  #options, args = parser.parse_args(args=sys.argv[1:])
    options.codes='sh600339,sh600556'
    print options,args
    ...
    #设置查询时间间隔（默认6秒）   -s
    #设置线程数（默认3）（如果有需要）   -t
    #查询 智慧农业 sz000816 例如: python stock_terminal.py - c sz000816 - t 4 - s3
    #支持查询多个股票 例如:python stock_terminal.py - c sh601003, sz000816, sz000778, ss600221
