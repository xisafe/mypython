...
def upload(local_dir,remote_dir,ssh):
...
def hive_ddl(db,tb_list,sqlfile='/home/bigdata/python/hive/ddl.sql',ssh_con=ssh_test):
    try:
        cons.get_hive_dml(db,tb_list)
        if upload('F:'+sqlfile,sqlfile,ssh_con)>0:
            cmd = ["hive -f "+sqlfile]#你要执行的命令列表
            sh_run(cmd,ssh_con,if_print=1)
    except Exception as e :
        print ('\n\t 创建表错误:\t',str(e))
...
if __name__=='__main__':
    ...
    upload('F:/home/bigdata/python/meta_satic.py',tar_path+'/test2.py',ssh)
    ssh.close()
    #ssh_test.close()
