...
def tb_to_csv(db='xfqz',tb='table_name',sql_txt='select 1',op_date=today):
    try:    
        ...
        print ('str(e):\t\t', str(e))        
def workers0():
    pro_data=list_sql[list_sql['groups']==0]
    for i in  pro_data.itertuples():
        db=i[4]
        tb=i[2]
        sql_txt=i[3]
        print('进程1：',tb)
        tb_to_csv(db,tb,sql_txt,op_date='20170919')
def workers1():
    pro_data=list_sql[list_sql['groups']==1]
    for i in  pro_data.itertuples():
        db=i[4]
        tb=i[2]
        sql_txt=i[3]
        ...
        tb_to_csv(db,tb,sql_txt,op_date='20170919')
def workers2():
    pro_data=list_sql[list_sql['groups']==2]
    for i in  pro_data.itertuples():
        db=i[4]
        tb=i[2]
        sql_txt=i[3]
        ...
        tb_to_csv(db,tb,sql_txt,op_date='20170919')
def workers3():
    pro_data=list_sql[list_sql['groups']==3]
    for i in  pro_data.itertuples():
        db=i[4]
        tb=i[2]
        sql_txt=i[3]
...
if __name__ == '__main__':
    ...
    list_sql['groups']=list_sql.index%4
    try:
        p0 = Process(target=workers0)
        p1 = Process(target=workers1)
        p2 = Process(target=workers2)
        p3 = Process(target=workers3) 
        p0.start()
        p1.start()
        p2.start()
        p3.start()
