...
if __name__ == '__main__':
    ...
    plt.show()
    plt.close()
    valid_data=norm_data[(norm_data['diff_weight']<=0.5)&(norm_data['diff_weight']>=-0.5)] #正常群体样本
    plotDist(valid_data['diff_weight'],title='',bins_k=20)
    ...
    groupBoxPlot(valid_data,valueby='diff_weight',groupby='hosts',top=10) 
    groupBoxPlot(valid_data,valueby='diff_weight',groupby='weight_or_vol',top=10) 
    #vendorname=valid_data.groupby(by='vendrname')['diff_rate'].describe().unstack()
    groupBoxPlot(valid_data,valueby='diff_weight',groupby='qty',top=10) 
    groupBoxPlot(valid_data,valueby='diff_weight',groupby='wharehouse',top=10) 
    #groupBoxPlot(valid_data,valueby='diff_rate',groupby='vendrname',top=10) 
    #plt.close()
    groupBoxPlot(valid_data,valueby='diff_weight',groupby='ez_dis_prod_catg_lvl_1_nm',top=40) 
    groupBoxPlot(valid_data,valueby='diff_weight',groupby='ez_dis_prod_catg_lvl_2_nm',top=80)
    plt.close()
    #sys.exit()
